...
function MotionDetector(configuration, jpegEventEmitter) {
	...
	if (configuration.showDeviation) {
		...
		this._processor.on("computations", function(results) {
			...
			var recording = (self._recordStream && self._recordTo + 500 >= dsl.getTime());

			process.stdout.write(dsl.toISOString() + " Deviation=" + results.stddev + "  " + (recording ? '(R)' : '') +
					"  [" + self._frames.length + "/" + self._maxImages + "]        \r");

			if (results.detected || recording) {
...
for ( var i in proto) {
	MotionDetector.prototype[i] = proto[i];
}
