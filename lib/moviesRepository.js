...
var proto = {
	...
	_listDirectoryContents: function(path, list, callback) {
		...
		fs.watch(path, function(event, filename) {
			console.log("Change event: ", filename, event);
	...
	_watchEvent: function(path, event, filename) {
		...
		setTimeout(function() {
			console.log("Change event: ", path, event, filename);

			fs.lstat(path, function(error, stats) {
				if (error) {
					console.error("lstat " + path + " error", error);
					return;
				}
				if (stats.isDirectory()) {
					return self._scanDirectories(path, function(error) {
						if (error) {
							console.error(error);
							return;
						}
				...
				if (stats.isFile()) {
					if (path.match(/\.mjpeg$/g)) {
						self._scanFiles([ path ], function(error) {
							if (error) {
								console.error(error);
								return;
							}
